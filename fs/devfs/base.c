...
static struct devfs_entry *
search_for_entry_in_dir(struct devfs_entry *parent,
                        const char *name,
                        unsigned int namelen,
                        int traverse_symlink)
{
    ...
    retval = curr->registered ?
              search_for_entry(parent, curr->u.symlink.linkname,
                               curr->u.symlink.length, FALSE, FALSE, NULL,TRUE)
                    : NULL;
...
static struct devfs_entry *
create_entry(struct devfs_entry *parent, const char *name,unsigned int namelen)
{
    ...
    if ((new = kmalloc(sizeof(*new) + namelen, GFP_KERNEL)) == NULL) return NULL;
...
static struct devfs_entry *get_root_entry(void)
{
    ...
    if ((new = create_entry(root_entry, ".devfsd", 0)) == NULL) return NULL;

    devnum = devfs_alloc_devnum(DEVFS_SPECIAL_CHR); // 申请一个空闲的设备号
...
static struct devfs_entry *search_for_entry(
    ...
    int traverse_symlink)
{
    ...
    while (subname < stop)
    {
        ...
        if (strncmp(subname, "../", 3) == 0)
        {
            /*  Going up  */
            dir = dir->parent;
            if (dir == NULL) /* Cannot escape from devfs */
                return NULL;
            subname += 3;
            continue;
        }
               ...
               umode_t mode, void *ops, void *info)
{
    ...
    if ((S_ISCHR(mode) || S_ISBLK(mode)) && (flags & DEVFS_FL_AUTO_DEVNUM))
    {
    ...
    de = search_for_entry(dir, name, strlen(name), TRUE, TRUE, &is_new, FALSE);
    if (de == NULL)
    {
        printk ("%s: devfs_register(): could not create entry: \"%s\"\n", DEVFS_NAME, name);
        if (devnum != NODEV)
            devfs_dealloc_devnum(devtype, devnum);
        return NULL;
    }
...
static int devfs_rmdir(struct inode *dir, struct dentry *dentry)
{
    ...
    devfsd_notify_one(de, DEVFSD_NOTIFY_DELETE, inode->i_mode, inode->i_uid, inode->i_gid, fs_info);
            ...
            int rdev)
{
    ...
    if (parent == NULL) return -ENOENT;
    /*  Try to reclaim an existing devfs entry, create if there isn't one  */
    de = search_for_entry(parent, dentry->d_name.name, dentry->d_name.len, FALSE, TRUE, &is_new, FALSE);
    if (de == NULL) return -ENOMEM;
    if (de->registered)
    {
    ...
    if ((inode = get_vfs_inode(dir->i_sb, de, dentry)) == NULL) return -ENOMEM;
...
#endif
    d_instantiate(dentry, inode);
    devfsd_notify_one(de, DEVFSD_NOTIFY_CREATE, inode->i_mode, inode->i_uid, inode->i_gid, fs_info);
    return 0;
